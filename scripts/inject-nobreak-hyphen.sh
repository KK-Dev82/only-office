...
#    ยังทำงานปกติแบบ default — แค่กลับไปตัดบรรทัดที่ขีดตามมาตรฐาน UAX #14)
...
# [3] ตอนเขียนไฟล์ docx — ของเดิม: ถ้าเป็นขีดกลางที่ห้ามตัด ให้เขียนออกเป็น <w:noBreakHyphen/>
STOCK_WRITE='e.prototype.xMf=function(){return!1===this.TOa()&&45===this.ma}'
# ของใหม่: ไม่เขียน element นี้ลงไฟล์เลย — เก็บเป็นตัวอักษร "-" ธรรมดาเสมอ
#   เหตุผล: <w:noBreakHyphen/> เป็น element ไม่ใช่ตัวอักษร เวลา copy จาก Word แล้ว paste
#   แบบ keep text only Word แปลงเป็น "เคาะวรรค" -> "402-403" กลายเป็น "402 403" ข้อมูลเปลี่ยนเงียบ ๆ
#   หน้าที่ห้ามตัดบรรทัดอยู่ที่ [1]+[2] ซึ่งทำงานตอน "วาด" อยู่แล้ว ไม่ต้องพึ่งของในไฟล์
PATCHED_WRITE='e.prototype.xMf=function(){return!1}'
...
patch_one() {
  ...
  replace_token "$f" "no-write" "$STOCK_WRITE" "$PATCHED_WRITE"; rc=$?
  [ "$rc" = "2" ] && return 1
  [ "$rc" = "0" ] && changed=1
